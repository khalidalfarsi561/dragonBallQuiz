    ...
    return [
      {
        ...
        headers: [
          ...
          // expand directives deliberately (e.g. add https: sources per directive).
          {
            ...
            value:
              "default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'none'; form-action 'self'; img-src 'self' data:; font-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; connect-src 'self' http://127.0.0.1:8090;",
          },
        ],
      },
    ];
